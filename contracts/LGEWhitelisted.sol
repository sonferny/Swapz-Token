...
contract LGEWhitelisted is Context {
    ...
    function _applyLGEWhitelist(address sender, address recipient, uint256 amount) internal {
        ...
        if(_lgeTimestamp == 0 && sender != _lgePairAddress && recipient == _lgePairAddress && amount > 0)
            _lgeTimestamp = now;
            //console.log("made it to time stamp");
        
        if(sender == _lgePairAddress && recipient != _lgePairAddress) {   //sender == _lgePairAddress && recipient != _lgePairAddress
            //console.log("made it to 3rd if statement");
            //buying
